...
**OpenTelemetry Collector** unifie la collecte de télémétrie : déployé comme un agent sur chaque nœud, il collecte les logs des conteneurs, les enrichit avec les métadonnées Kubernetes (namespace, pod, etc.), et les envoie vers un backend de stockage. Il sert de pipeline standardisé pour les logs, métriques et traces, évitant d'avoir des agents séparés pour chaque signal. Cela permet de passer d'une alerte à la preuve opérationnelle : est-ce que l'application logue encore correctement après correction ? est-ce qu'un événement de sécurité Falco correspond à des logs applicatifs ?
